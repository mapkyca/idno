                ...
                if ($content = \Idno\Core\Webservice::get($u)['content']) {
                    ...
                    if ($return = $parser->parse()) {
                        ...
                        if (isset($return['items'])) {
                            ...
                            if (!count($hcard)) {
                                //throw new \Idno\Exceptions\KnownException("Sorry, could not find any users on that page, perhaps they need to mark up their profile in <a href=\"http://microformats.org/wiki/microformats-2\">Microformats</a>?"); // TODO: Add a manual way to add the user
                        ...
                        throw new \Idno\Exceptions\KnownException("Sorry, there was a problem parsing the page!");
                } else
                    throw new \Idno\Exceptions\KnownException("Sorry, $u could not be retrieved!");
            ...
            function postContent()
            {
                ...
                if ($uuid = $this->getInput('uuid')) {
                    ...
                    if ($new_user) {
                        ...
                        throw new \Idno\Exceptions\KnownException('Sorry, that user doesn\'t exist!');
                } else
                    throw new \Idno\Exceptions\KnownException("No UUID, please try that again!");
            }
