                ...
                if (file_exists($file_path) && $path = \Idno\Core\Idno::site()->config()->uploadpath) {
                    ...
                    try {
                        ...
                        if (!@copy($file_path, $upload_file)) {
                            throw new \Idno\Exceptions\FilesystemException("There was a problem storing the file data.");
                        }
                        if (!@file_put_contents($data_file, $metadata)) {
                                throw new \Idno\Exceptions\FilesystemException("There was a problem saving the file's metadata");
                        }
